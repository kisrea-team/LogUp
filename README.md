...
| :--- | :---: | :---: | :--- |
| **基础能力 (Core Logic)** | 6.0 | 8.0 | 版本提取是核心强项（注册表/GitHub 确定性 + LGB 排序 + LLM 兜底，80 例 ~89%）。依赖/组件版本混淆、JS 渲染页仍是明确短板 |
...
**总评**：护城河是版本提取质量（~89% 不虚），运营形成"提取→交叉校验→人工审核→负样本回流"闭环。但仍是"单服务 + 定时脚本 + 人工审核"形态——**引擎强（6/10）、平台弱（3.5~4/10），初-中级运营系统**，补上工业化、反爬、高可用才能规模化。

> 本表由 deepseek-v4-flash0731 评估；LogUp 维度分基于实测数据，评分不因评估模型署名而浮动。
