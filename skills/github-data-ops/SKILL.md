...
> ⚠️ **最高优先级强制要求**：每个项目的 `links` 字段第一条必须是 changelog 来源（`type: "changelog"`），其余链接必须包含 **至少 3 条非官方中文链接**（非官网、非官方文档），来自知乎、少数派、B站、掘金、CSDN 等中文社区。目标总链接数 5-9 条。**禁止收录 V2EX 链接。仅有官方链接或链接数量不足视为未完成工作。**
...
2. **存量审查**：遍历现有项目，执行以下检查并直接修复：
   - `summar` 或 `describe` 非中文 → 翻译为中文并更新
   - `describe` 有官方原文可获取 → 优先使用官方原文翻译，而非 AI 自创内容
   - `tags` 为空或标签数量不足 → 根据项目特性补充 2-5 个中文标签
   - `links` 为空 → 主动搜索并补充至少 3 条相关资源链接，第一条必须为 changelog 来源
   - `links[0]` 不是 changelog 类型或为空 → 补充 changelog 来源链接至第一位
   - 更新版本时：**优先访问 `links[0]`（changelog 来源）获取最新版本信息**，无需重新搜索
   - `links[0]` 维护：更新已有项目时，用 WebFetch 验证 `links[0].url` 是否可访问；若返回 404/403 或无法加载，重新查找正确的 changelog 来源并替换 `links[0]`
3. **存量评估**：对现有项目进行运营评估，自主决策是否删除质量低劣、信息过时、无实际价值的项目
4. **决策选品**：按 30/70 策略决定本次收录内容
5. **爬取信息**：自行获取项目版本、更新日志等信息
6. **执行操作**：通过 API 完成添加/更新/删除，每次新增不少于 5 个项目；**每完成一个项目后立即输出该操作结果，不要等所有项目处理完再统一汇报**
7. **收尾汇总**：所有操作完成后，输出本次运营的简要统计（新增/更新/删除各几条）
...
| 值 | 含义 |
|----|------|
| `changelog` | **更新日志来源**（GitHub Releases、官网 Changelog 页等，固定用于第一条） |
...
**收录规范：**
- **`links[0]` 固定为更新日志来源**（`type: "changelog"`）：填写获取版本更新信息的地址，如 GitHub Releases 页、官网 Changelog 页、RSS Feed 等。此链接将用于后续版本更新时优先爬取，不得省略。
- 每个项目目标收录 **3-9 条**不同类型链接（含第一条 changelog），尽力搜集 5 条以上
- **必须包含至少 3 条非官方链接**（非官网、非官方文档），来自中文社区
...
```json
[
  { "title": "GitHub Releases", "url": "https://github.com/owner/repo/releases", "type": "changelog" },
...
- **links 非官方链接（强制）**：每个项目 `links` 必须包含 **至少 3 条非官方中文链接**，且 `links[0]` 必须为 changelog 来源。目标 5-9 条，链接须为具体文章/视频/帖子页面（禁止搜索结果页、禁止 V2EX 链接）
