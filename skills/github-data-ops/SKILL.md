...
> ⚠️ **最高优先级强制要求**：每个项目的 `links` 字段必须包含 **至少 3 条非官方中文链接**（非官网、非官方文档），来自知乎、少数派、B站、掘金、V2EX、CSDN 等中文社区。**不允许只放 2 条了事**，目标是 4-6 条有价值的链接。仅有官方链接或链接数量不足视为未完成工作。
...
**收录规范：**
- 每个项目目标收录 **2-8 条**不同类型链接，**严禁偷懒只放 2 条**，尽力搜集 4 条以上
...
### describe 字段
详情页展示，必须为中文。两段内容**必须用以下格式明显区分**，不可合并成一段：

**第一段（社区洞察，如能获取）**：
主动搜索 Reddit、Hacker News、V2EX 等社区中关于该项目的讨论，提炼社区真实声音。
...
Outline 是一款开源的团队知识库工具，支持 Markdown 编辑...
```
...
- 若搜索无结果或项目过于小众，可跳过此段（直接从「项目介绍」开始）

**第二段（项目描述）**，固定以 `**📖 项目介绍**` 开头：
...
- **links 非官方链接（强制）**：每个项目 `links` 必须包含 **至少 3 条非官方中文链接**，严禁只放官方文档或 2 条了事。目标 4-6 条，链接须为具体文章/视频/帖子页面（禁止搜索结果页）
