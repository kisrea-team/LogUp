...
### 1. 强制熔断触发器
* **检测点**：AI 在计划搜索 `[项目名] 教程/评测` 之前，必须先回答：**“该项目的最新版本号是多少？数据来源在哪？”**
* **惩罚性逻辑**：若日志中未出现 `curl .../releases/latest` 或同等版本查询指令，则后续所有 `search` 指令均视为无效浪费，必须立即停止。

### 2. 录入原子化指令 (Atomic Operation)
* **不可分割性**：`POST /projects` 和 `POST /versions` 必须被视为同一个逻辑块。
* **录入流程**：
    1. 获取项目名 & 检查库内存量。
    2. **获取版本号 (必须有有效数字版本)** -> **未通过则此处终止**。
    3. 获取 3 条中文链接。
    4. 顺序执行两个 POST 请求。
