...
2. **按需更新/补齐存量 (优先级：补债 > 更新)**
   ...
   - **no-cache 智能筛查**：若提示词中列出了"no-cache 项目"（`update_source_url` 服务器不返回缓存头，无法自动检测变化），**不可逐一全量检查**，须用以下多种信号快速预判，只对高概率更新项目做完整版本核查，其余跳过：
     1. **新闻搜索**：对每个 no-cache 项目执行 `/search/news?q={项目名} release`，若近期（1个月内）有发布动态则列为高优先级；
...
| `update_source_url` | String? | **新增项目时必填**。获取版本更新信息的首选 URL（GitHub Releases 页、官网 Changelog、RSS Feed 等）。预检脚本每次运行前会对此 URL 发 HEAD 请求：若 ETag/Last-Modified 有变化则提示优先更新（changed）；若服务器不返回缓存头则标记为 no-cache 并交由 AI 多信号智能筛查；字段为空的项目不参与预检，需每次全量检查。 |
...
> **`update_source_url` 必填说明**：新增项目时必须在 POST body 中传入此字段，缺少此字段视为未完成，须补填后再提交。更新已有项目时若尚未填写，也请通过 `PUT /api/projects/{id}` 补填。系统会在每次运营前对该 URL 发 HEAD 请求：ETag/Last-Modified 变化时自动标记为 changed 提示优先检查；服务器不返回缓存头时标记为 no-cache 交由 AI 多信号智能筛查；两种情况均好于字段为空（只能全量扫描）。
