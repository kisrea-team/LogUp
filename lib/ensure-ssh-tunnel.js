...
function ensureSshTunnel() {
  ...
  server.on('error', (err) => {
    if (err.code === 'EADDRINUSE') {
      console.warn(`SSH Tunnel warning: Port ${port} is already in use. Assuming it is occupied by a parallel build worker or previous instance.`);
      // Keep the lock so we don't try again
